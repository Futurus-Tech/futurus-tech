        ...
        <div className="mt-gap-lg border-t-2 border-divider pt-5">
          ...
          <TeamGrid>
            {content.team.map((member) => (
              <figure key={member.id} data-stagger-item className="group m-0">
                {/* The print is black and white until the pointer settles on it,
                    which is the one place the page lets a photograph carry its
                    own colour. `--photo-grayscale` is the same handle the hero
                    reel and the case cards drive, so the drain is one value
                    changing and `filter` transitions across it. */}
                <Photo
                  className="aspect-4/5 border-2 border-divider bg-surface transition-[filter] duration-500 ease-out group-hover:[--photo-grayscale:0]"
                ...
                <figcaption className="mt-3.5 border-t-2 border-divider pt-3">
                  <p className="m-0 font-heading text-brand-sm font-extrabold transition-colors duration-300 group-hover:text-accent">
                    {member.name}
                  </p>
